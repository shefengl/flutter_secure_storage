...
class FlutterSecureStorage {
  ...
  Future<String> read({@required String key, IOSOptions iOptions, AndroidOptions aOptions}) async {
    final String value = await _channel.invokeMethod(
        'read', <String, dynamic>{'key': key, 'options': _selectOptions(iOptions, aOptions)});
    return value;
  }
  ...
  Future<void> delete({@required String key, IOSOptions iOptions, AndroidOptions aOptions}) =>
      _channel.invokeMethod(
          'delete', <String, dynamic>{'key': key, 'options': _selectOptions(iOptions, aOptions)});
  ...
  Future<Map<String, String>> readAll({IOSOptions iOptions, AndroidOptions aOptions}) async {
    final Map results = await _channel
        .invokeMethod('readAll', <String, dynamic>{'options': _selectOptions(iOptions, aOptions)});
  ...
  /// [aOptions] optional Android options
  /// Can throw a [PlatformException].
  Future<void> deleteAll({IOSOptions iOptions, AndroidOptions aOptions}) => _channel
      .invokeMethod('deleteAll', <String, dynamic>{'options': _selectOptions(iOptions, aOptions)});
  ...
  Future<String> contain(
      {@required String key, IOSOptions iOptions, AndroidOptions aOptions}) async {
    final String value = await _channel.invokeMethod(
        'contain', <String, dynamic>{'key': key, 'options': _selectOptions(iOptions, aOptions)});
    return value;
  }
}
